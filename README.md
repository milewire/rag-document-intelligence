...
- Single-page frontend at root (`/`) with document list, chunk counts, delete-from-sidebar, and dynamic suggested prompts
- Upload and query errors always return JSON (no HTML error pages)
...
Use `encodeURIComponent` for filenames with spaces or special characters (e.g. `My%20file.pdf`).
